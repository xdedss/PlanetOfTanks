...
//#endif
	...
	{//important! Vertex
		...
		posWorld += float4(normalWorld * dist / 800, 0);
	...
	)
	{
		...
		//float depth;
		//float3 normalValues;
		//DecodeDepthNormal(tex2D(_CameraDepthNormalsTexture, scrpos), depth, normalValues);
		...
		if (LinearEyeDepth(i.pos.z) - LinearEyeDepth(depth)  > 5) {
			discard;
		}
		...
		if (LinearEyeDepth(i.pos.z) - LinearEyeDepth(depth)  > 2) {
			//discard;
			//s.diffColor = float4(1,0,1,1);
		}
		//s.diffColor = (depth) * float3(1,1,1);
